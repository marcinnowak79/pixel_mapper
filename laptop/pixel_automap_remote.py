...
HOST = '192.168.1.201'  # The server's hostname or IP address
...
cap = cv2.VideoCapture(0)  # buid in macBook pro camera
...
class PolygonDrawer(object):
    ...
    def run(self):
        ...
        while (not self.done):
            ...
            cv2.resizeWindow(self.window_name, h_res, v_res);
...
def turn_on(indexString):
    # turn on pixel at index
    # TODO: implement me
    s.sendall(bytes('ON '+str(indexString), 'utf-8'))
    time.sleep(0.2)
    print("********* on 1 pixel")
...
cv2.resizeWindow("Camera1", h_res, v_res);

for index in range(0, totalpixels):
    attempts = 1
    while attempts > 0:
        pixelout = {}
        all_off()
        ...
        if len(cnts) > 0:
            ...
            if len(cnts) == 1:
                ...
                vsource.outputpoints[0 * 170 + index] = [0 * 170 + index, cX, cY]
            else:
        ...
        if attempts >= 2:
            ...
            playsound('alert.wav')
            cv2.imshow("Camera1", image)
            # TODO: maybe remove this! converting all unum (universum numeber? dont use uinversum to 0)
            cv2.setMouseCallback("Camera1", on_mouse, [0 * 170 + index, vsource])
            done = 0
            while done == 0:
                cv2.waitKey(50)
                if vsource.outputpoints[0 * 170 + index] != [0, 0, 0]:
                    done = 1
                else:
                    done = 0
            attempts = 0
    with open('data1.txt', 'a') as outfile:
        outfile.write(str(0 * 170 + index) + ',' + str(vsource.outputpoints[0 * 170 + index][1]) + ',' + str(
            vsource.outputpoints[0 * 170 + index][2]) + "\n")
all_off()
